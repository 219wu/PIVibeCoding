...
rem    vibekit open-watch       open new window watch (title=task name), alias ow
rem    vibekit task <file>      start task from requirements file
rem    vibekit metrics
...
echo available: watch ^| open-watch ^| task ^| dashboard ^| metrics ^| summary ^| checkpoint ^| vibe_state ^| security ^| adr ^| init
goto :eof
